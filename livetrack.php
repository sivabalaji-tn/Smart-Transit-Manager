...
<div class="container py-3">
  ...
  <div class="main-container">
    <div class="row g-3">
      <div class="col-lg-8 col-12"><div id="map"></div></div>
      <div class="col-lg-4 col-12">
        <div id="log-panel" aria-live="polite" aria-atomic="true">
          <h6 class="text-warning"><i class="bi bi-clock-history"></i> LIVE LOGS</h6>
          <div id="log-feed" class="mt-2"><div class="text-muted small">Waiting for bus to start...</div></div>
        </div>
      </div>
...
function initMapAndRoute(){
  ...
  const BusIcon=L.divIcon({className:'transparent-icon',html:'<i class="bi bi-bus-front-fill text-danger fs-3"></i>',iconSize:[30,30],iconAnchor:[15,30]});
  const StopIcon=L.divIcon({className:'transparent-icon',html:'<i class="bi bi-geo-alt-fill text-warning fs-5"></i>',iconSize:[20,20],iconAnchor:[10,20]});
  ...
  stops.forEach((s,i)=>{
    ...
    marker.bindPopup(`<strong>${escapeHtml(s.stop_name)}</strong><br/>Stop ${s.sequence_number??(i+1)}`);
  ...
  window.BusIcon=BusIcon; // expose for later
}
...
function updateBusLocation(){
  ...
  .then(data=>{
    ...
    if(!isNaN(lat)&&!isNaN(lon)&&lat!==0&&lon!==0){latestLocation={lat,lon};}
    ...
    if(status!=='Active'||isNaN(lat)||isNaN(lon)||lat===0||lon===0){
      ...
      if(prevStatus!==status){addLog(`<span class="text-danger">STATUS:</span> Bus is ${escapeHtml(status)}.`,'danger');}
      prevStatus=status;
      return;
    }
    ...
    if(busMarker){
      ...
      busMarker=L.marker(latLng,{title:'Bus Current Location',icon:window.BusIcon}).addTo(map);
      busMarker.bindPopup(`<strong>Bus Location</strong><br/>${new Date(ts).toLocaleTimeString()}`).openPopup();
    }

    if(busMarkerCircle){busMarkerCircle.setLatLng(latLng);}
    else{busMarkerCircle=L.circle(latLng,{radius:12,color:'#0d6efd',fillColor:'#0d6efd',fillOpacity:0.9}).addTo(map);}

    stops.forEach((s,i)=>{
      if(stopVisited[i])return;
      ...
      if(d<=arrivalDistanceMeters){
        ...
        stopVisited[i]=true;
      }else if(d<=arrivalDistanceMeters*2&&!s.approachLogged){
    ...
    if(emergencyFlag){
      ...
      setTimeout(()=>map.removeLayer(emMarker),45000);
    }
  })
  .catch(err=>{
...
document.getElementById('emergency-btn').addEventListener('click',()=>{
  let lat=latestLocation.lat,lon=latestLocation.lon;
  if(!lat||!lon){
...
function reportEmergency(lat,lon){
  ...
  fetch(apiReportEmergency,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bus_id:busId,lat,lon})})
  .then(r=>r.json())
  .then(resp=>{
    if(resp?.success){
  ...
  setTimeout(()=>map.removeLayer(emMarker),60000);
}
